...
/*Home Section 1*/
.welcome_section_area{background-color:<?php echo $complete['welcome_bgcolor_id'] ?>;}
.welcome_section_area_bg{background:url(<?php echo $complete['welcome_bg_image'] ?>);}
...
.featured_block{background-color:<?php echo $complete['featured_block_bg'] ?>; border-color:<?php echo $complete['featured_block_bd'] ?>;}


.sktmore, .wpcf7-submit{background-color:<?php echo $complete['featured_block_button_bg'] ?>;}
.sktmore:hover, .wpcf7-submit:hover{background-color:<?php echo $complete['featured_block_hover_button_bg'] ?>;}
...
/*-----------------------------COLORS------------------------------------*/
		...
		.header{background-image:url(<?php echo $complete['header_bg_setting'] ?>) !important; }

		<?php if(!empty($complete['header_bg_setting'])){ ?>
		.home.has_trans_header .layer_wrapper .header{position:relative !important;}
		.home.has_trans_header .nivo-caption {padding:25px 0 !important;}
		<?php } ?>
		
		.page .header, .single .header, .archive .header, .search .header{ position:relative!important; background-color: rgba(<?php echo complete_hex2rgb($complete['head_color_id']);?>, 1)!important;}
		...
		<?php } ?>
		.home.has_trans_header.page .header{background:<?php echo $complete['head_color_id']; ?>!important;}
		...
		.logo h1, .logo h1 a:link, .logo h1 a:visited, .logo h1 a:hover{ 
			<?php if(!empty($logofont['font-family'])){ ?>font-family:'<?php echo $logofont['font-family']; ?>'; 
			<?php } ?>
		...
		.logo > h1 > a > span, #footer .footercols1 h3::first-letter, #footer .footercols2 h3::first-letter, #footer .footercols3 h3::first-letter, #footer .footercols4 h3::first-letter, h1.postitle::first-letter, .single_post_content h1::first-letter, h2.postitle::first-letter{
			color:<?php echo $complete['first_logo_color_id']; ?>;
		}
		...
		.title{ 
			...
			background-color:<?php echo $complete['slidetitle_bg_color_id']; ?>;
		}	
		
		.title{background-color: rgba(<?php echo complete_hex2rgb($complete['slidetitle_bg_color_id']);?>, <?php if(empty($complete['slider_bg_trans'])){ echo '1'; }else { echo $complete['slider_bg_trans'];} ?>);}
		...
		.slidedesc{ 
			...
			background-color:<?php echo $complete['slddesc_bg_color_id']; ?>;
		}	
		.slidedesc{background-color: rgba(<?php echo complete_hex2rgb($complete['slddesc_bg_color_id']);?>, <?php if(empty($complete['slider_bg_trans'])){ echo '1'; }else { echo $complete['slider_bg_trans'];} ?>);}
				
		<?php if((!empty($complete['slider_upcase_id']))){ ?>
		...
		.slidebtn a{ 
			color:<?php echo $complete['sldbtntext_color_id']; ?>; 
			background-color: rgba(<?php echo complete_hex2rgb($complete['sldbtn_color_id']);?>, <?php if(empty($complete['slider_bg_trans'])){ echo '1'; }else { echo $complete['slider_bg_trans'];} ?>);}	
		.slidebtn a:hover{background-color: rgba(<?php echo complete_hex2rgb($complete['sldbtn_hvcolor_id']);?>, <?php if(empty($complete['slider_bg_trans'])){ echo '1'; }else { echo $complete['slider_bg_trans'];} ?>);
		...
		.social-icons a:link, .social-icons a:visited{ 
			border-color:<?php echo $complete['social_icon_color_id']; ?>;
			color:<?php echo $complete['social_text_color_id']; ?>;
		}	
		
		/* Social Icon Hover Background Color */
		.social-icons a:hover{ 
			border-color:<?php echo $complete['social_hover_icon_color_id']; ?>;
			color:<?php echo $complete['social_hover_text_color_id']; ?>;
		}
		...
		#topmenu ul.sub-menu li a:hover, #topmenu li.menu_highlight_slim:hover{background-color:<?php echo $complete['mnshvr_color_id']; ?> !important;}	
...
<?php if($complete['sec_color_id']){ ?>
		...
		.icon_round a, #wp-calendar #today, .moretag:hover, .search_term #searchsubmit, .error_msg #searchsubmit, .complete_pagenav a:hover, .ast_login_widget #loginform #wp-submit, #searchsubmit, .prog_wrap, .rel_tab .related_img i, .lay1 h2.postitle a, .nivoinner .slide_button_wrap .lts_button, #accordion .slide_button_wrap .lts_button, .lts_layout1 .icon_wrap a, .lts_layout2 .icon_wrap a, .lts_layout3 .icon_wrap a, .lts_layout1 .icon_wrap a:hover, input[type=submit]{color:<?php echo $complete['sectxt_color_id']; ?>;}
		.thn_post_wrap .listing-item .moretag:hover, body .lts_layout1 .listing-item .title, .lts_layout2 .img_wrap .complete_plus, .img_hover .icon_wrap a, body .thn_post_wrap .lts_layout1 .icon_wrap a, .wpcf7-submit, .woo-slider #post_slider li.sale .woo_sale, p.form-submit #submit, .optimposts .type-product a.button.add_to_cart_button, input[type=submit]:hover{color:<?php echo $complete['sectxt_color_id']; ?>;}
...
.divider1, .divider2, .divider3, .divider4, .divider5, .divider6, .fusion-title .title-sep, .fusion-title.sep-underline, .product .product-border, .fusion-title .title-sep, .fusion-title2 .title-sep2{border-color:<?php echo $complete['divider_color_id'] ?>;}
...
#footer .footercols1, #footer .footercols2, #footer .footercols3, #footer .footercols4, .footer-post-area ul li h6, .footer-post-area ul li h6 a:link, .footer-post-area ul li h6 a:visited{color:<?php echo $complete['footwdgtxt_color_id']; ?>;}
.footer-post-area ul li{border-color:<?php echo $complete['footer_title_color']; ?>;}
