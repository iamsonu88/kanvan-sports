...
if ( ! function_exists( '_wp_render_title_tag' ) ) {
	function complete_wp_title( $title, $sep ) {
		...
		if ( $paged >= 2 || $page >= 2 )
			$title = "$title $sep " . sprintf( __( 'Page %s', 'sktgravida' ), max( $paged, $page ) );
   ...
   extract( shortcode_atts( array(
	...
	if ( have_posts() ) :
		while ( have_posts() ) : the_post();
			...
			$postoutput .= '<li><a href="'.get_the_permalink().'">'.$imgUrl.'</a><h6><a href="'.get_the_permalink().'">'.get_the_title().'</a></h6><p>'.wp_trim_words( get_the_content(), 20, '' ).'</p></li>';
		endwhile;
	endif;
	wp_reset_query();
	$postoutput .= '</ul></div>';
