<?php 
/**
 * The SHARE THIS Function for gravida
 *
...
global $complete;?>
        ...
        <div id="ast_nextprev" class="navigation <?php if (empty($complete['post_nextprev_id'])){ echo 'hide_nextprev'; }?>">
            ...
            <?php $prevPost = get_previous_post(true); if($prevPost) {?>
                ...
                <?php previous_post_link('%link','<i class="fa fa-angle-left"></i>'.$prevthumbnail.''.__('Previous Post','sktgravida').'<br><span>%title</span>', TRUE); ?>
            ...
            <?php $nextPost = get_next_post(true); if($nextPost) { ?>
                ...
                <?php next_post_link('%link','<i class="fa fa-angle-right"></i>'.$nextthumbnail.''.__('Next Post','sktgravida').'<br><span>%title</span>', TRUE); ?>
